...
public class regionRecognition {
    private static int checkNeighbour(int[][] array,List<int[]> currentRegion, int[] currentPixel, int row,int column){
        ...
        if(newColumn<0 || newRow<0 || newRow>=array[0].length || newColumn>= array.length){
            return 0;
        }
    ...
    public static void main(String[] args){
        int[][] input= {{1,0,1},{1,0,0},{0,2,0}}; //input[i][j] -> i-th row, j-th column
